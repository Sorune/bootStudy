...
import com.sorune.bootstudy.entity.QMember;
import com.sorune.bootstudy.entity.QReply;
...
public class SearchBoardRepositoryImpl extends QuerydslRepositorySupport implements SearchBoardRepository{
    ...
    public Board search1() {
        ...
        jpqlQuery = from(board);
        jpqlQuery.leftJoin(reply).on(reply.board.eq(board));

        QMember member = QMember.member;

        JPQLQuery<Board> jpqlQuery1 = from(board);
        jpqlQuery1.leftJoin(member).on(board.writer.eq(member));
        jpqlQuery1.leftJoin(reply).on(reply.board.eq(board));
        jpqlQuery1.select(board,member.email,reply.count()).groupBy(board);
        log.info(jpqlQuery1);

        List<Board> result1 = jpqlQuery1.fetch();
